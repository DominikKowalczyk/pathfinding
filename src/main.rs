// src/main.rs
use image::{GrayImage, DynamicImage, Rgba, RgbaImage, io::Reader as ImageReader};
...
impl Ord for Node {
    fn cmp(&self, other: &Self) -> Ordering {
        other.priority.partial_cmp(&self.priority).unwrap_or(Ordering::Equal) // Reverse order for min-heap
    }
}
...
fn validate_grayscale_image(img: &DynamicImage) -> Result<(), Error> {
    match img {
        ...
        _ => Err(Error::msg("Heightmap must be a grayscale image.")),
    }
}

/// Calculates the effort required to move between two points, considering terrain factors.
fn effort(from: u8, to: u8, distance: f64, weight: f64, fatigue_factor: f64, temperature: f64) -> f64 {
...
fn main() -> Result<(), Error> {
    ...
    if let Some((cost, path)) = find_optimal_path(&gray_img, start, goal, args.weight, args.fatigue_factor, args.temperature) {
        info!("Optimal path cost: {:.2}", cost);

        // Convert the grayscale image to an RGB image to draw the path
        let (width, height) = (gray_img.width(), gray_img.height());
        info!("Grayscale image dimensions: {}x{}", width, height);

        let mut img_rgb = RgbaImage::new(width, height);
        for (x, y, pixel) in img_rgb.enumerate_pixels_mut() {
            let gray = gray_img.get_pixel(x, y)[0];
            *pixel = Rgba([gray, gray, gray, 255]);
        }

        // Draw the path on the image
        for (x, y) in &path {
            img_rgb.put_pixel(*x, *y, Rgba([255, 0, 0, 255])); // Red color for the path
        }

        // Save the image with the path
        img_rgb.save("pathfinding_result.png")?;
